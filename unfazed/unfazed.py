...
def summarize_record(read_record, include_ambiguous, verbose, evidence_min_ratio):
    ...
    elif (mom_read_count > 0) and (mom_read_count >= evidence_min_ratio * dad_read_count):
        origin_parent = read_record["mom"]
        other_parent = read_record["dad"]
    ...
    if (dad_cnv_site_count > 0) and (dad_cnv_site_count >= evidence_min_ratio * mom_cnv_site_count):
    ...
    elif (mom_cnv_site_count > 0) and (mom_cnv_site_count >= evidence_min_ratio * dad_cnv_site_count):
...
def write_vcf_output(in_vcf_name, read_records, include_ambiguous, verbose, outfile, evidence_min_ratio):
    vcf = VCF(in_vcf_name)
...
def write_bed_output(read_records, include_ambiguous, verbose, outfile, evidence_min_ratio):
    header = [
        "#chrom",
        "start",
        "end",
    ...
    for key in read_records:
        record_summary = summarize_record(read_records[key], include_ambiguous, verbose, evidence_min_ratio)
...
def unfazed(args):
    ...
    if len(svs) > 0:
        phased_svs = phase_svs(
            svs, kids, pedigrees, args.sites, args.threads, args.build, args.no_extended, args.multiread_proc_min, args.ab_homref, args.ab_homalt, args.ab_het, args.min_gt_qual, args.min_depth, args.search_dist, args.insert_size_max_sample, args.stdevs, args.min_map_qual, args.readlen, args.split_error_margin
        )
    if len(snvs) > 0:
        phased_snvs = phase_snvs(
            snvs, kids, pedigrees, args.sites, args.threads, args.build, args.no_extended, args.multiread_proc_min, args.ab_homref, args.ab_homalt, args.ab_het, args.min_gt_qual, args.min_depth, args.search_dist, args.insert_size_max_sample, args.stdevs, args.min_map_qual, args.readlen, args.split_error_margin
        )
    ...
    if output_type == "vcf":
        write_vcf_output(
            args.dnms, all_phased, args.include_ambiguous, args.verbose, args.outfile, args.evidence_min_ratio
        )
    elif output_type == "bed":
        write_bed_output(
            all_phased, args.include_ambiguous, args.verbose, args.outfile, args.evidence_min_ratio
        )
